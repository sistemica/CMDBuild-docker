...
RUN mkdir -p shared/classes && mkdir -p shared/lib
RUN mkdir -p /data
...
COPY files/apps/cmdbuild-2.3.4.war /data/cmdbuild-2.3.4.war
...
RUN /etc/init.d/postgresql start && \
    ...
    export PGPASSWORD=cmdbuildsu  
    #su postgres -c "psql -U cmdbuildsu -h localhost -d cmdbuildsu -a -f /tmp/initialize_db.sql"
#
# Create data directory for Alfresco, start tomcat and deploy cmdbuild
RUN mkdir -p /var/alfresco/data 
